...
const applyLanguage = (language) => {
  ...
  const translations = {
    ...
    'nav a[href="Dashboard.html"]': text.viewStudent,
    'nav a[href="AddStudent.html"]': text.addStudent,
    'nav a[href="Quiz.html"]': text.quizGame,
    'nav a[href="StudentGrades.html"]': text.studentGrades,
    "#logout": text.logout,
    'a.back[href="Dashboard.html"]': text.backDashboard,
    'a[href="Dashboard.html"].secondary': text.goBack,
...
const setupDashboard = () => {
  ...
  if (!isAdmin) {
    document
      .querySelectorAll('nav a[href="AddStudent.html"], .add')
  ...
  document.getElementById("studentRows").addEventListener("click", (event) => {
    ...
    if (button.dataset.action === "edit-student") {
      location.href = `EditStudent.html?uid=${encodeURIComponent(button.dataset.uid)}`;
      return;
    }
...
const setupAddStudent = () => {
  ...
  if (!requireAdmin("Only the admin can add students.", "Dashboard.html"))
    return;
...
const setupEditStudent = () => {
  ...
  setupLogout();
  const user = currentUser();
  if (!requireAdmin("Only the admin can edit students.", "Dashboard.html"))
    return;
  setupAdminPicture(user);
...
const setupAdminSettings = () => {
  ...
  const user = currentUser();
  if (
    !requireAdmin("Only the admin can open admin settings.", "Dashboard.html")
  )
    return;
  setupAdminPicture(user);
  document.getElementById("userName").textContent =
...
const setupQuiz = () => {
  ...
  if (user.role === "student") {
    document
      .querySelectorAll('nav a[href="AddStudent.html"]')
      .forEach((element) => element.remove());
  }
